...
from keras.layers import Input, Add, PReLU, Conv2DTranspose, Concatenate, MaxPooling2D, UpSampling2D, Dropout, Dense, LeakyReLU, Activation, Flatten, Reshape
...
class L0Loss:
    def __init__(self):
        self.gamma = tf.Variable(2.0)

    def __call__(self):
        def calc_loss(y_true, y_pred):
            loss = tf.pow(tf.abs(y_true - y_pred) + 1e-8, self.gamma)
            return loss
        return calc_loss
...
class L1Loss:
    ...
    def __call__(self):
        def calc_loss(y_true, y_pred):
            loss = tf.abs(y_true - y_pred) + 1e-8
            return loss
        return calc_loss
...
class UpdateAnnealingParameter(Callback):
    ...
    def on_epoch_begin(self, epoch, logs=None):
        ...
        tf.keras.backend.set_value(self.gamma, new_gamma)
